...
var kriging = function () {
    ...
    kriging.plotAQI = function (canvas, grid, xlim, ylim) {
    ...
    kriging.plotAQIWithGradient = function (canvas, grid, xlim, ylim) {
        var gradient = [{
            value: 0,
            r: 125,
            g: 228,
        ...
        for (i = 0; i < n; i++)
            for (j = 0; j < m; j++) {
                ...
                y = canvas.height * (1 - (j * grid.width + grid.ylim[0] - ylim[0]) / range[1]);
                z = grid[i][j];
                for (var k = 1; k < gradient.length; k++) {
                    if (z <= gradient[k].value) {
                        r = gradient[k].r - Math.round((gradient[k].value - z) * (gradient[k].r - gradient[k - 1].r) / (gradient[k].value - gradient[k - 1].value));
                        g = gradient[k].g - Math.round((gradient[k].value - z) * (gradient[k].g - gradient[k - 1].g) / (gradient[k].value - gradient[k - 1].value));
                        b = gradient[k].b - Math.round((gradient[k].value - z) * (gradient[k].b - gradient[k - 1].b) / (gradient[k].value - gradient[k - 1].value));
                        break;
                    }
                }
