...
namespace SAW.Core.InterpolationAlgorithm
{
    public class KrigingSrc
    {
        ...
        // Variogram models
        static double KrigingVariogramGaussian(double h, VariogramSrc variogram)
        {
        ...
        static double KrigingVariogramExponential(double h, VariogramSrc variogram)
        {
            return variogram.Nugget + ((variogram.Sill - variogram.Nugget) / variogram.Range) * (1.0 - Math.Exp(-h / variogram.Range / variogram.A));
        }

        static double KrigingVariogramSpherical(double h, VariogramSrc variogram)
        {
        ...
        public static VariogramSrc Train(double[] t, double[] x, double[] y, KrigingModelSrc model, double sigma2, double alpha)
        {
            VariogramSrc variogram = new VariogramSrc(t, x, y);
            switch (model)
            {
                case KrigingModelSrc.Gaussian:
                    {
                        variogram.Model = KrigingVariogramGaussian;
                        break;
                    }
                case KrigingModelSrc.Exponential:
                    {
                        variogram.Model = KrigingVariogramExponential;
                        break;
                    }
                case KrigingModelSrc.Spherical:
                    {
        ...
        // Model prediction
        public static double Predict(double x, double y, VariogramSrc variogram)
        {
        ...
        public static double Variance(double x, double y, VariogramSrc variogram)
        {
            double[] k = new double[variogram.N];
            int i;
            for (i = 0; i < variogram.N; i++)
        ...
        public static void Grid(double[][][] polygons, VariogramSrc variogram, double width)
        {
    ...
    public class VariogramSrc
    {
        ...
        public Func<double, VariogramSrc, double> Model { get; set; }
        ...
        public VariogramSrc(double[] t, double[] x, double[] y) : this()
        {
            T = t;
            X = x;
            Y = y;
